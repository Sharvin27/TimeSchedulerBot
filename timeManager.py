...
YOUR_CALENDAR_ID = os.getenv('YOUR_CALENDAR_ID')
...
def commitHours(creds):
    try:
        ...
        events_result = service.events().list(calendarId=YOUR_CALENDAR_ID, 
                                              timeMin=timeStart, timeMax=timeEnd, 
                                              singleEvents=True, orderBy='startTime', 
                                              timeZone=YOUR_TIMEZONE).execute()
...
def addEvent(creds, duration, description):
    ...
    service = build('calendar', 'v3', credentials=creds)
    event = service.events().insert(calendarId=YOUR_CALENDAR_ID, 
                                    body=event).execute()
